...
impl Api {
    ...
    pub fn build(self, bearer_code: &str) -> RequestBuilder {
        ...
        if let Some(comment_ids) = self.comment_ids {
            query_parameters.push(("comment_ids", format!("[\"{}\"]", comment_ids.join("\",\""))));
        }
...
impl Data {
    pub fn is_empty_extra(&self) -> bool {
        let res = self.extra.is_empty() &&
        self.comments.as_ref().map(|it| it.iter().all(|item| item.is_empty_extra())).unwrap_or(true);
        if !res {
